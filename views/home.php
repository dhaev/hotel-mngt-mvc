<?php
require_once __DIR__ . '/../config/config.php'; // Use BASE_PATH for consistent file inclusion
...
$numDays = (strtotime($checkout) - strtotime($checkin)) / (60 * 60 * 24);
   ...
   <form id="textForm" action="<?= BASE_URL?>index.php?controller=Booking&action=add" method="post">
      <div class="form-row justify-content-center dates">
         <div class="form-group col-md-4">
            ...
            <input type="date" class="form-control date" name="checkin" id="cin" value="<?= $checkin ?>" required>
         </div>
         <div class="form-group col-md-4">
            <label for="cout">Check out</label>
            <input type="date" class="form-control date" name="checkout" id="cout" value="<?= $checkout ?>" required>
         </div>
      </div>
...
document.addEventListener('DOMContentLoaded', function () {
    function calculatePrice(row) {
        ...
        const checkout = new Date(document.getElementById('cout').value);
        const numDays = Math.max((checkout - checkin) / (1000 * 60 * 60 * 24), 1); // Ensure at least 1 day
